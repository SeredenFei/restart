...
## Project Defaults: Graduation Thesis Route

Inside `Restart_Graduation`, interpret this pipeline as a graduation-thesis workflow centered on YOLO-based weed detection.

Default local stage mapping:

1. topic framing and literature scan
2. task definition and technical route
3. dataset inspection and split design
4. baseline model selection
5. YOLO training and debugging
6. experiment comparison and ablation
7. thesis chapter drafting
8. Chinese polishing / de-AIGC cleanup
9. defense PPT preparation

When a request is thesis-oriented rather than publication-oriented, prefer the local `project-skills/*` chain over journal-style review loops.
