...
## Project Defaults: YOLO Training Workflow

Inside this repository, prioritize PyTorch guidance for YOLO-style training and debugging.

Default check order:

1. image and label pairing
2. YOLO annotation format
3. class index consistency
4. train/val split leakage
5. batch size, image size, and GPU memory fit
6. overfitting on small field datasets

## Local Output Rules

- Prefer the smallest fix that can be verified quickly.
- Explain hyperparameters in a way that can be reused in the thesis experiment chapter.
- Focus on reproducibility, not abstract framework discussion.
